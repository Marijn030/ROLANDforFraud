...
class GTLayer(nn.Module):
        ...
        **kwargs,
        ...
        if global_model_type == 'None':
            ...
            for edge_type in metadata[1]:
                ...
                self.e_lin[edge_type_str] = Linear(dim_in, dim_h)
                self.g_lin[edge_type_str] = Linear(dim_h, dim_out)
                self.oe_lin[edge_type_str] = Linear(dim_h, dim_out)

            H, D = self.num_heads, self.dim_h // self.num_heads
            if cfg.gt.edge_weight:
                self.edge_weights = nn.Parameter(
                    torch.Tensor(len(metadata[1]), H, D, D)
                )
                self.msg_weights = nn.Parameter(
                    torch.Tensor(len(metadata[1]), H, D, D)
                )
            ...
            for edge_type in metadata[1]:
                edge_type_str = '__'.join(edge_type)
                self.k_lin[edge_type_str] = Linear(dim_in, dim_h)
                self.q_lin[edge_type_str] = Linear(dim_in, dim_h)
                self.v_lin[edge_type_str] = Linear(dim_in, dim_h)
                self.e_lin[edge_type_str] = Linear(dim_in, dim_h)
                self.g_lin[edge_type_str] = Linear(dim_h, dim_out)
                self.oe_lin[edge_type_str] = Linear(dim_h, dim_out)
                self.o_lin[edge_type_str] = Linear(dim_h, dim_out)
            ...
            raise ValueError(f'Unsupported global model type: {global_model_type}')
        ...
        for edge_type in metadata[1]:
            edge_type_str = "__".join(edge_type)
            if self.layer_norm:
                self.norm1_edge_local[edge_type_str] = nn.LayerNorm(dim_h)
                self.norm1_edge_global[edge_type_str] = nn.LayerNorm(dim_h)
            if self.batch_norm:
                self.norm1_edge_local[edge_type_str] = nn.BatchNorm1d(dim_h)
                self.norm1_edge_global[edge_type_str] = nn.BatchNorm1d(dim_h)
        ...
        if cfg.gt.ffn == 'Single':
            ...
            for edge_type in metadata[1]:
                edge_type_str = "__".join(edge_type)
                self.ff_linear1_edge_type[edge_type_str] = nn.Linear(
                    dim_h, dim_h * 2
                )
                self.ff_linear2_edge_type[edge_type_str] = nn.Linear(
                    dim_h * 2, dim_h
                )
    ...
    def forward(self, batch):
        ...
        if isinstance(batch, HeteroData):
            ...
            edge_types = list(batch.edge_types)
            h_dict = batch.collect('x')
        ...
        # Disable raw edge features for this ROLAND integration.
        # The current dataset has edge_feature dim 2, while GT hidden dim is
        # cfg.gt.dim_hidden (e.g. 32). FraudGT's sparse edge-feature path
        # expects hidden-size edge embeddings, so until an edge projection
        # layer is added, ignore raw edge features.
        has_edge_attr = False
        ...
        if self.global_model_type != 'None':
            if self.layer_norm or self.batch_norm:
                ...
                if has_edge_attr:
                    edge_attr_dict = {
                        edge_type: self.norm1_edge_global["__".join(edge_type)](
            ...
            if self.global_model_type == 'TorchTransformer':
                ...
                if cfg.gt.attn_mask in ['Edge', 'kHop']:
                    attn_mask = torch.full(
                        (L, S), -1e9, dtype=torch.float32, device=edge_index.device
                    )
                    if cfg.gt.attn_mask == 'kHop':
                        ...
                        for idx, edge_index_k in enumerate(reversed(edge_index_list)):
                            attn_mask[edge_index_k[1, :], edge_index_k[0, :]] = self.bias[idx]
                    else:
                ...
                h, attn_weights = self.attn(
                    q, k, v, attn_mask=attn_mask, need_weights=True
                )
                saved_scores = attn_weights
                h = h.view(1, -1, D)

                for idx, node_type in enumerate(node_types):
                    out_type = h[:, node_type_tensor == idx, :]
                    h_attn_dict_list[node_type].append(out_type.squeeze(0))
                ...
                if isinstance(batch, HeteroData):
                    ...
                    homo_x = batch.node_feature
                    edge_index = batch.edge_index
                    node_type_tensor = torch.zeros(
                        homo_x.shape[0], dtype=torch.long, device=homo_x.device
                    )
                    edge_type_tensor = torch.zeros(
                        edge_index.shape[1], dtype=torch.long, device=edge_index.device
                    )

                q = torch.empty((homo_x.shape[0], self.dim_h), device=homo_x.device)
                k = torch.empty((homo_x.shape[0], self.dim_h), device=homo_x.device)
                v = torch.empty((homo_x.shape[0], self.dim_h), device=homo_x.device)
                edge_attr = torch.empty(
                    (edge_index.shape[1], self.dim_h), device=homo_x.device
                )
                edge_gate = torch.empty(
                    (edge_index.shape[1], self.dim_h), device=homo_x.device
                )
                ...
                if has_edge_attr:
                    for idx, edge_type_tuple in enumerate(edge_types):
                        ...
                        edge_attr[mask] = self.e_lin[edge_type_str](
                            edge_attr_dict[edge_type_tuple]
                        )
                        edge_gate[mask] = self.g_lin[edge_type_str](
                            edge_attr_dict[edge_type_tuple]
                        )
                ...
                if has_edge_attr:
                    edge_attr = edge_attr.view(-1, H, D).transpose(0, 1)
                    edge_gate = edge_gate.view(-1, H, D).transpose(0, 1)

                q = q.view(-1, H, D).transpose(0, 1)
                k = k.view(-1, H, D).transpose(0, 1)
                v = v.view(-1, H, D).transpose(0, 1)
                ...
                if cfg.gt.attn_mask in ['Edge', 'kHop']:
                    if cfg.gt.attn_mask == 'kHop':
                        ...
                        for idx, edge_index_k_item in enumerate(reversed(edge_index_list)):
                            attn_mask[
                                edge_index_k_item[1, :], edge_index_k_item[0, :]
                            ] = self.bias[idx]
                    ...
                    if hasattr(self, 'edge_weights'):
                        edge_weight = self.edge_weights[edge_type_tensor]
                        edge_weight = edge_weight.transpose(0, 1)
                        edge_k = edge_k.unsqueeze(-1)
                        edge_k = torch.matmul(edge_weight, edge_k).squeeze(-1)
                    ...
                    edge_scores = torch.sum(edge_scores, dim=-1) / math.sqrt(D)
                    ...
                    max_scores, _ = scatter_max(
                        edge_scores, expanded_dst_nodes, dim=1, dim_size=L
                    )
                    ...
                    edge_scores = exp_scores / sum_exp_scores.gather(
                        1, expanded_dst_nodes
                    )
                    edge_scores = self.dropout_attn(edge_scores.unsqueeze(-1))
                ...
                for idx, node_type in enumerate(node_types):
                    ...
                    h_attn_dict_list[node_type].append(out_type)
                ...
                if has_edge_attr:
                    ...
                    for idx, edge_type_tuple in enumerate(edge_types):
                        edge_type_str = '__'.join(edge_type_tuple)
                        mask = edge_type_tensor == idx
                        out_type = self.oe_lin[edge_type_str](edge_attr[mask, :])
                        edge_attr_dict[edge_type_tuple] = out_type
            else:
                raise ValueError(
                    f'Unsupported global model type: {self.global_model_type}'
                )

            h_attn_dict = {}
            for node_type in h_attn_dict_list:
                h_attn_dict[node_type] = torch.sum(
                    torch.stack(h_attn_dict_list[node_type], dim=0), dim=0
                )
            ...
            if cfg.gt.residual == 'Fixed':
                ...
                alpha_dict = {
                    node_type: self.skip_global[node_type].sigmoid()
                    for node_type in node_types
                }
                h_attn_dict = {
            ...
            h_out_dict_list = {
                node_type: h_out_dict_list[node_type] + [h_attn_dict[node_type]]
                for node_type in node_types
            }

        h_dict = {}
        for node_type in node_types:
            if len(h_out_dict_list[node_type]) == 0:
                h_dict[node_type] = h_in_dict[node_type]
            else:
                h_dict[node_type] = sum(h_out_dict_list[node_type])
        ...
        if cfg.gt.ffn != 'none':
            ...
            if cfg.gt.ffn == 'Type':
                h_dict = {
                    node_type: h_dict[node_type]
                    + self._ff_block_type(h_dict[node_type], node_type)
                    for node_type in node_types
                }
                if has_edge_attr:
                    edge_attr_dict = {
                        edge_type: edge_attr_dict[edge_type]
                        + self._ff_block_edge_type(edge_attr_dict[edge_type], edge_type)
                        for edge_type in edge_types
                    }
        ...
        if isinstance(batch, HeteroData):
            ...
            if has_edge_attr:
                batch.edge_feature = edge_attr_dict[edge_types[0]]
